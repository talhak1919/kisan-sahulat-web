import {
  ...
  export const createQuestion = (question) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(`/api/v1/farmer-forum/create-question`, question, config);
      dispatch({
  ...
  export const getAllQuestions = () => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(`/api/v1/farmer-forum/all-questions`);
...
export const updateAnswers = (id, answersData) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.put(
        `/api/v1/farmer-forum/update-answers/${id}`,
        answersData,
        config
      );
